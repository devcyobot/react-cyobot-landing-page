...
export default async function useBlogContentByID(id: string): Promise<WebsiteBlog | null> {
    "use server"; // Enables SSR for this function

    try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_DASHBOARD_URL}/api/website-blogs/${id}`, {
